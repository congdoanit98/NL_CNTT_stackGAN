...
class Image_data:
    ...
    def image_processing(self, filename, vector):
        ...
        if self.augment_flag:
            ...
            img = tf.cond(pred = tf.greater_equal(tf.random_uniform(shape = [], minval= 0.0, maxval = 1.0), 0.5), true_fn = lambda : augmentation(img, augment_height_size, augment_width_size),false_fn = lambda : img)
...
def show_all_variables():
    ...
    slim.model_analyzer.analyzer_vars(model_vars, print_info = True)
