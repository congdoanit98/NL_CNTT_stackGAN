...
class stackGAN():
    def __init__(self, sess, args):
        ...
        print("--- Discriminator ---")
        print("# special normalization: ", self.sn)
    ...
    # GENERATOR
    def generator_1(self, text_embedding, noise, is_training = True, reuse = tf.AUTO_REUSE, scope = 'generator_2'):
        ...
        with tf.variable_scope(scope, reuse = reuse):
            ...
            x = conv(x, channels = self.img_ch, kernel = 3, stride = 1, pad = 1, pad_style = 'reflect', use_bias = True, sn = self.sn, scope = 'g_logit')
            x = tanh(x)
    ...
    def generator_2(self, x_init, text_embedding, is_training = True, reuse = tf.AUTO_REUSE, scope = 'generator_2'):
        ...
        with tf.variable_scope(scope, reuse = reuse):
            ...
            x = conv(x_init, channels, kernel = 3, stride = 1, pad = 1, pad_style = 'reflect', use_bias = True, sn = self.sn, scope = 'conv')
            x = relu(x)

            for i in range(2):
                x = conv(x, channels * 2, kernel = 4, stride = 2, pad = 1, pad_style = 'reflect', use_bias = False, sn = self.sn, scope = 'conv_'+str(i))
            ...
            x = conv(x, channels, kernel = 3, stride = 1, pad = 1, pad_style = 'reflect', use_bias = False, sn = self.sn, scope = 'joint_conv')
    ...
    def discriminator_1(self, x_init, mu, is_training = True, reuse = tf.AUTO_REUSE, scope = "discriminator_1"):
        channels = self.df_dim

        with tf.variable_scope(scope, reuse = reuse):
            x = conv(x_init, channels, kernel = 4, stride = 2, pad = 1, pad_style = 'reflect', use_bias = True, sn = self.sn, scope = 'conv')
            x = relu(x, 0.2)
            ...
            x = conv(x, channels, kernel = 3, stride = 1, pad = 1, pad_style = 'reflect', use_bias = False, sn = self.sn, scope = 'conv_last')
    ...
    def discriminator_2(self, x_init, mu, is_training = True, reuse = tf.AUTO_REUSE, scope = "discriminator_2"):
        ...
        with tf.variable_scope(scope, reuse = reuse):
            ...
            for i in range(2):
                x = conv(x, channels // 2, kernel = 3, stride = 1, pad = 1, pad_style = 'reflect', use_bias = False, sn = self.sn, scope = 'conv3x3_' + str(i))
    ...
    def build_model(self):
        if self.phase == 'train':
            ...
            g_summary_list = [self.summary_g_loss_64, self.summary_g_loss_256, self.summary_g_adv_loss_64, self.summary_g_adv_loss_256, self.summary_g_kl_loss_64, self.summary_g_kl_loss_256]
            d_summary_list = [self.summary_d_loss_64, self.summary_d_loss_256, self.summary_d_adv_loss_64, self.summary_d_adv_loss_256]
    ...
    def train(self):
        ...
        for stage in range(init_stage, 2):
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                # Update G
                real_images, fake_images, _, g_loss, summary_str = self.sess.run([self.real_img[stage], self.fake_img[stage], self.g_optim[stage], self.g_loss[stage], self.summary_merge_g_loss], feed_dict = train_feed_dict)
                ...
                if np.mod(idx + 1, self.print_freq) == 0:
                    ...
                    save_images(merge_images, [1, 2], './{}/merge_stage{}_{:07d}.jpg'.format(self.sample_dir, stage, idx + 1))
    ...
    def save(self, checkpoint_dir, step):
        ...
        if not os.path.exits(checkpoint_dir):
    ...
    def load(self, checkpoint_dir):
        ...
        ckpt = tf.train.get_checkpoint_stage(checkpoint_dir)
        if ckpt and ckpt.model_checkpoint_path:
            ...
            print(" [!] Filed to find a checkpoint")
            return False, 0
    ...
    def test(self):
        ...
        for i in range(5):
            ...
            real_images = np.expand_dims(real_images[i], axis = 0)
            fake_images = np.expand_dims(fake_images[i], axis = 0)

            save_images(real_images, [1, 1], real_path)
            save_images(fake_images, [1, 1], fake_path)

            index.write("<td>%s</td>"% os.path.basename(real_path))
            index.write("<td><img src = '%s' width = '%d' height = '%d'></td>" % (real_path if os.path.isabs(real_path) else('../..' + os.path.sep + real_path), self.img_width, self.img_height))
            index.write("<td><img src = '%s' width = '%d' height = '%d'></td>" % (fake_path if os.path.isabs(fake_path) else('../..' + os.path.sep + fake_path), self.img_width, self.img_height))
            index.write("</tr>")
